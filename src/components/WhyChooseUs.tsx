...
export const WhyChooseUs: React.FC = () => {
  return (
    <section id="why-us" className="py-16 sm:py-24 bg-[#0F172A] relative overflow-hidden">
      
      {/* Background ambient lighting */}
      <motion.div
        animate={{ scale: [1, 1.15, 1], opacity: [0.1, 0.2, 0.1] }}
        transition={{ duration: 9, repeat: Infinity, ease: 'easeInOut' }}
        className="absolute top-1/3 right-0 w-96 max-w-full h-96 bg-orange-600/15 rounded-full blur-[140px] pointer-events-none"
      />
      <motion.div
        animate={{ scale: [1.1, 1, 1.1], opacity: [0.15, 0.05, 0.15] }}
        transition={{ duration: 11, repeat: Infinity, ease: 'easeInOut' }}
        className="absolute bottom-10 left-10 w-96 max-w-full h-96 bg-amber-500/10 rounded-full blur-[140px] pointer-events-none"
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          {WHY_CHOOSE_US.map((item: FeatureItem, index: number) => {
            ...
            return (
              <motion.div
                ...
                className="p-5 sm:p-7 rounded-2xl glass-card border border-slate-800 hover:border-orange-500/40 hover:shadow-xl hover:shadow-orange-950/20 transition-all duration-300 space-y-4 group relative overflow-hidden"
              >
                ...
                <h3 className="text-base sm:text-lg font-bold text-white group-hover:text-amber-300 transition-colors font-['Poppins']">
                  {item.title}
                </h3>
        ...
        <motion.div
          ...
          className="mt-12 sm:mt-16 grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 p-5 sm:p-8 rounded-3xl bg-slate-900/80 border border-slate-800 shadow-xl text-center"
        >
          <div className="space-y-1">
            <h4 className="text-xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-300">
              500+
            </h4>
            <p className="text-xs text-slate-400 font-medium">Successful Trips</p>
          </div>
          <div className="space-y-1">
            <h4 className="text-xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-yellow-300">
              100%
            </h4>
            <p className="text-xs text-slate-400 font-medium">On-Time Record</p>
          </div>
          <div className="space-y-1">
            <h4 className="text-xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-300">
              5.0 ★
            </h4>
            <p className="text-xs text-slate-400 font-medium">Customer Rating</p>
          </div>
          <div className="space-y-1">
            <h4 className="text-xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-rose-400">
              100%
            </h4>
